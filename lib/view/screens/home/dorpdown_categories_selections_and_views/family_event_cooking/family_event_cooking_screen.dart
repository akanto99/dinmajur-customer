...
class _FamilyEventCookingScreenState extends State<FamilyEventCookingScreen> {
  ...
  // REGULAR: categoryId -> packageId
  Map<String, String?> _selectedPackages = {};

  // MANUAL: categoryId -> Set of "packageId_itemId"
  Map<String, Set<String>> _selectedManualItems = {};

  // CUSTOM uses _selectedPackages (same as REGULAR, one package per category)
  ...
  void _toggleManualItemSelection(String categoryId, String packageId, String itemId) {
    ...
    setState(() {
      ...
      if (_selectedManualItems[categoryId]!.contains(key)) {
        ...
        if (_selectedManualItems[categoryId]!.isEmpty) _activeCategoryId = null;
      } else {
  ...
  bool _isPackageSelected(String categoryId, String packageId) => _selectedPackages[categoryId] == packageId;

  bool _isManualItemSelected(String categoryId, String packageId, String itemId) => _selectedManualItems[categoryId]?.contains('${packageId}_$itemId') ?? false;

  bool _canSelectFromCategory(String categoryId) => _activeCategoryId == null || _activeCategoryId == categoryId;
                ...
                : NotificationListener<ScrollNotification>(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        final isSelected = _selectedGuestRangeIndex == index;
                        return SizedBox(width: itemWidth - (8 / visibleCount), child: _buildRangeTab(index, guestRanges[index].label ?? '', isSelected));
                      },
                    ),
                  ),
          ),
  ...
  Widget _buildRangeTab(int index, String label, bool isSelected) {
    return GestureDetector(
      onTap: () => setState(() => _selectedGuestRangeIndex = index),
      child: Container(
      ...
      child: Column(
        ...
        children: [
          ...
          ...((package.items ?? []).asMap().entries.map((entry) {
            ...
            return _buildManualItemCard(category, package, entry.value, screenWidth, isLastItem);
          })),
        ],
      ),
    );
