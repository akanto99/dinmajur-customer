...
class _CustomDrawerState extends State<CustomDrawer> {
          ...
          child: Column(
            ...
            children: [
              GestureDetector(
                ...
                child: Container(height: 60, color: AppColors.containerBackground(context), child: AppBarHeader(AppLocalizations.of(context)!.profile_menu)),
              ),
              Expanded(
          ...
          case Status.COMPLETED:
            ...
            if (profileViewModel.profileviewUserData.data?.data?.user == null) {
              return SizedBox(
                ...
                child: Center(child: Text(AppLocalizations.of(context)!.no_data_found)),
              );
            }
  ...
  Widget _buildDrawerItem(IconData icon, String title) {
    return Container(
      ...
      decoration: BoxDecoration(
        border: Border(bottom: BorderSide(width: 1.0, color: AppColors.border(context))),
      ),
      child: Row(
  ...
  Future<void> _showLogoutDialog() async {
    ...
    return showDialog<void>(
      ...
      builder: (BuildContext dialogContext) {
        // Use dialogContext for dialog-specific context
        return Consumer<LoginLogoutViewModel>(
          // ✅ Wrap with Consumer
          builder: (context, loginLogoutViewModel, child) {
            return AlertDialog(
              backgroundColor: AppColors.appBackground(context),
              ...
              content: Column(
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      GestureDetector(
                        onTap: loginLogoutViewModel.loggingOut
                            ? null // Disable cancel during logout
                            : () => Navigator.of(dialogContext).pop(),
                        child: Container(
                          width: screenWidth * 0.25,
                          height: 38,
                          decoration: BoxDecoration(
                            color: loginLogoutViewModel.loggingOut ? AppColors.textFieldFill(context).withOpacity(0.5) : AppColors.textFieldFill(context),
                      ...
                      GestureDetector(
                        onTap: loginLogoutViewModel.loggingOut
                            ? null // Disable tap during logout
                            : () async {
                                await _performLogout(dialogContext); // Pass dialogContext
                              },
                        child: Container(
                          width: screenWidth * 0.25,
                          height: 38,
                          ...
                          decoration: BoxDecoration(color: AppColors.button(context), borderRadius: BorderRadius.circular(8)),
                          child: Center(
                            child: loginLogoutViewModel.loggingOut
                                ? LoadingAnimationWidget.progressiveDots(color: AppColors.whiteColor, size: 20) // ✅ Show loading animation
                                : Text(
  ...
  Future<void> _performLogout(BuildContext dialogContext) async {
    try {
      ...
      // ✅ Close dialog after successful logout (before navigation)
  ...
  void _callprofileImageAPI() {
    if (_selectedProfileImageData != null && _selectedProfileImageName != null) {
      ...
      patchImagePostViewModel.profileImageUpdatePatchApi(_selectedProfileImageData!, _selectedProfileImageName!, "profilePicture", 1, context);
    }
  }
}
