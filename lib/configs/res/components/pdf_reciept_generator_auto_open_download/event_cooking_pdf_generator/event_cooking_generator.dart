...
class UniversalServiceItem {
  ...
  final List<UniversalSubItem> items;
  final int quantity;
  final num totalPrice;
...
// ==================== UNIVERSAL PDF GENERATOR WITH BENGALI SUPPORT ====================

class UniversalPDFReceiptGenerator {
  // Cache for fonts to avoid loading them multiple times
  static pw.Font? _bengaliFont;
  static pw.Font? _bengaliFontBold;

  /// Load Bengali fonts (HindSiliguri from your assets)
  static Future<void> _loadBengaliFonts() async {
    if (_bengaliFont == null) {
      try {
        final fontData = await rootBundle.load('assets/fonts/hind-siliguri/HindSiliguri-Regular.ttf');
  ...
  /// Generate and download receipt PDF with Bengali support
  static Future<File?> generateAndDownloadPDFReceipt(UniversalReceiptData receiptData) async {
    try {
      // Load fonts first
      await _loadBengaliFonts();
  ...
  // ==================== PDF Components with Bengali Font Support ====================
  ...
  static pw.Widget _buildServicesSection(UniversalReceiptData data) {
    if (data.services.isEmpty) {
      return pw.Container(
        ...
        child: pw.Text(
          'No services available',
          style: pw.TextStyle(font: _bengaliFont, fontSize: 14, color: PdfColors.grey),
        ),
      );
    }
                ...
                children: [
                  pw.TableRow(
                    ...
                    children: [
                      pw.Padding(
                        padding: pw.EdgeInsets.all(8),
                        child: pw.Text('Service', style: pw.TextStyle(font: _bengaliFontBold, fontSize: 12, fontWeight: pw.FontWeight.bold)),
                      ),
                      pw.Padding(
                        padding: pw.EdgeInsets.all(8),
                        child: pw.Text('Items', style: pw.TextStyle(font: _bengaliFontBold, fontSize: 12, fontWeight: pw.FontWeight.bold)),
                      ),
                      pw.Padding(
                        padding: pw.EdgeInsets.all(8),
                        child: pw.Text('Total\n(BDT)', style: pw.TextStyle(font: _bengaliFontBold, fontSize: 12, fontWeight: pw.FontWeight.bold), textAlign: pw.TextAlign.center),
                      ),
                    ],
                  ),
                ],
        ...
        ...data.services.map((service) {
          List<pw.Widget> itemWidgets = service.items.map((item) {
            return pw.Padding(
              ...
              child: pw.Text(
                '${item.name}${item.price > 0 ? ' - BDT ${item.price.toStringAsFixed(0)}' : ''}',
                style: pw.TextStyle(font: _bengaliFont, fontSize: 10),
              ),
            );
          }).toList();
                  ...
                  children: [
                    pw.TableRow(
                      children: [
                        pw.Padding(
                          padding: pw.EdgeInsets.all(8),
                          child: pw.Text(service.serviceName, style: pw.TextStyle(font: _bengaliFontBold, fontSize: 11, fontWeight: pw.FontWeight.bold)),
                        ),
                        pw.Padding(
                          padding: pw.EdgeInsets.all(8),
                          child: pw.Column(
                            crossAxisAlignment: pw.CrossAxisAlignment.start,
                            children: itemWidgets.isEmpty
                                ? [pw.Text('No items', style: pw.TextStyle(font: _bengaliFont, fontSize: 10, color: PdfColors.grey))]
                                : itemWidgets,
                          ),
                        ),
                        pw.Padding(
                          padding: pw.EdgeInsets.all(8),
                          child: pw.Text('BDT\n${service.totalPrice.toStringAsFixed(0)}', style: pw.TextStyle(font: _bengaliFontBold, fontSize: 11, fontWeight: pw.FontWeight.bold), textAlign: pw.TextAlign.center),
                        ),
                      ],
                    ),
                  ],
            ...
            pw.Padding(
              ...
              child: pw.Row(
                ...
                children: [
                  ...
                  pw.Text(
                    'BDT ${payment.grandTotal.toStringAsFixed(0)}',
                    style: pw.TextStyle(font: _bengaliFontBold, fontSize: 14, fontWeight: pw.FontWeight.bold),
                  ),
                ],
              ),
            ),
      ...
      child: pw.Column(
        children: [
          ...
          if (data.contactNumber != null) ...[
            ...
            pw.Text(
              'Contact us: ${data.contactNumber}',
              style: pw.TextStyle(font: _bengaliFont, fontSize: 12),
              textAlign: pw.TextAlign.center,
            ),
          ],
        ],
  ...
  static pw.Widget _buildDetailRow(String label, String value) {
    return pw.Row(
      ...
      children: [
        pw.Container(
          width: 120,
          child: pw.Text(label, style: pw.TextStyle(font: _bengaliFontBold, fontSize: 12, fontWeight: pw.FontWeight.bold)),
        ),
        pw.Expanded(
          child: pw.Text(value, style: pw.TextStyle(font: _bengaliFont, fontSize: 12)),
        ),
      ],
    );
  }
